...
- **Filesystem access via Effect `@effect/platform` `FileSystem`, not raw `node:fs` or a shell.** All reads go through the Effect `FileSystem` service (`readFileString`, `readDirectory`, `stat`, `realPath`), provided by a platform layer (see next decision). Each tool's logic is therefore an `Effect` that stays in the Effect world for I/O, error handling, and testability, and is only turned into a Promise at the LangChain boundary. Rationale: consistent with the rest of the app (Effect services + typed errors), avoids a second I/O style, and keeps path/error handling uniform. We still implement the *matching* logic (grep/find/tree/head/tail) ourselves on top of these primitives. Alternatives — raw `node:fs` (rejected: bypasses Effect, inconsistent error handling) or spawning `bash`/binaries (rejected: shell-injection risk and `tree` not installed by default on macOS).
- **Platform layer: `@effect/platform-node` (`NodeFileSystem` + `NodePath`), not `@effect/platform-bun`.** Although the runtime is Bun, we provide the `FileSystem`/`Path` services via `@effect/platform-node` subpath layers (`@effect/platform-node/NodeFileSystem`, `@effect/platform-node/NodePath`) merged with `Layer.merge`, which run fine under Bun via Node compatibility. **Caught during implementation:** the originally-planned `@effect/platform-bun@0.91.2` is entangled with the Effect *cluster* monorepo — its `peerDependencies` include `@effect/cluster`, `@effect/rpc`, and `@effect/sql`, and importing its barrel (`BunContext`) or `BunPath` pulls in `@effect/cluster/MessageStorage` and a mismatched `@effect/platform-node-shared`, breaking module resolution at load time. `@effect/platform-node` avoids those heavy peers and boots cleanly. Note: some standalone scripts hit a Bun subpath-resolution quirk for `@effect/platform/Error` depending on import order; the running application is unaffected (it boots and long-polls), and priming that import first resolves it in test harnesses.
...
- **Effect platform packaging churn** → `@effect/platform-bun` currently drags in the Effect cluster stack (cluster/rpc/sql peers) and breaks at import; mitigated by using `@effect/platform-node` under Bun. Trade-off: a "node" package on a Bun runtime, which works via Node compatibility.
...
Additive. Deploy by optionally setting `SANDBOX_ROOT` (defaults to the current directory) and installing `@effect/platform` + `@effect/platform-node` and `zod`. Rollback: build the agent with `tools: []` again; no persistent state is involved.
